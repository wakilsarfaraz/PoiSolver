...
for (n in 1:NTRI)
  ...
  for (i in 1:3){
    LV[LNODES[n,i]] = LV[LNODES[n,i]]+ F[i]
    }
	}
for (i in 1:NNODES){
if (x[i]==0 | y[i]==0 | x[i]==xmax | y[i]==xmax){
    LV[i] = 0
    SP[i,] = 0
    SP[i,i] = 1
     }
...
write.table(x,file="/Users/sarfaraw/Dropbox/ICGS/PIQ/R-Tutorial/Laplace/coordx.txt",row.names=FALSE,col.names=FALSE)
write.table(y,file="/Users/sarfaraw/Dropbox/ICGS/PIQ/R-Tutorial/Laplace/coordy.txt",row.names=FALSE,col.names=FALSE)
write.table(U,file="/Users/sarfaraw/Dropbox/ICGS/PIQ/R-Tutorial/Laplace/numsol.txt",row.names=FALSE,col.names=FALSE)
write.table(LNODES,file="/Users/sarfaraw/Dropbox/ICGS/PIQ/R-Tutorial/Laplace/triangles.txt",row.names=FALSE,col.names=FALSE)
